...
use super::{lyrics_manager::LyricsManager, scanner, utils::get_lyrics_async};
...
pub async fn handle_player_update(
    ...
    let Some((info, _)) = available_players.get_mut(&bus_name) else {
        ...
        tracing::info!(
            "[DEBUG] Player {} not found in available_players, returning early",
            bus_name
        );
        return;
    };
    ...
    if lyrics_manager.current_player.as_ref().map(|p| &p.bus) == Some(&bus_name) {
        ...
        if scanner::is_player_active(info) {
            ...
            if let Some(current_player) = &lyrics_manager.current_player {
                let is_at_end =
                    current_player.next_lrc_timetag == TimeTag::from(Duration::from_secs(u64::MAX));
            ...
            let is_loop_restart = lyrics_manager.detect_loop_restart(
                current_pos,
                is_position_update,
                is_metadata_update,
            );
            ...
            tracing::info!(
                "[DEBUG] Loop detection result: is_loop_restart = {}",
                is_loop_restart
            );

            if is_loop_restart {
                let prev_pos_secs = lyrics_manager
                    .last_known_position
                    .map(|p| p.as_secs())
                    .unwrap_or(0);
                tracing::info!(
                    "Detected song loop restart: {} - current: {}s, previous: {}s",
                    bus_name,
                    current_pos.as_secs(),
                    prev_pos_secs
                );
                ...
                tracing::debug!(
                    "Position tracking: {} - current: {}s, needs_reload: {}",
                    bus_name,
                    current_pos.as_secs(),
                    needs_reload
                );
        ...
        if let Some(Ok(lrc)) =
            get_lyrics_async(info, external_lrc_providers, navidrome_config).await
        {
            let track_id = info
                .metadata
                .get("mpris:trackid")
