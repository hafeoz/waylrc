...
pub fn handle_loop_check_timer(
    ...
    for (bus_name, (info, _)) in available_players.iter() {
        ...
        let status = info.status.as_ref().map(|s| format!("{:?}", s)).unwrap_or_else(|| "Unknown".to_string());

        // Only output debug info when there's a current player
        let is_current_player = lyrics_manager.current_player
            .as_ref()
            .map_or(false, |current_player| current_player.bus == *bus_name);

        if is_current_player {
            tracing::debug!("Current player {}: raw={}s, calc={}s, status={}",
                          bus_name, raw_pos.as_secs(), calc_pos.as_secs(), status);
        }
        ...
        if let Some(current_player) = &lyrics_manager.current_player {
            if current_player.bus == *bus_name {
                ...
                let should_refresh_due_to_loop = if let Some(last_loop_count) = lyrics_manager.last_loop_count {
                    if current_loop_count > last_loop_count {
                        tracing::info!("[LOOP COUNT] Detected song loop: {} -> {}, position in loop: {}s",
                                     last_loop_count, current_loop_count, position_in_loop.as_secs());
                        true
                    } else {
                        false
                    }
                ...
                let should_refresh_due_to_position = if current_player.next_lrc_timetag == TimeTag::from(Duration::from_secs(u64::MAX)) {
                    ...
                    if let Some(last_pos) = lyrics_manager.last_known_position {
                        ...
                        if last_pos > current_pos && (last_pos - current_pos) > Duration::from_secs(30) {
                ...
                if should_refresh_due_to_loop || should_refresh_due_to_position {
                    let track_id = info.metadata.get("mpris:trackid")
    ...
    if let Some((bus_name, position_in_loop)) = reset_player_position {
        if let Some((info, _)) = available_players.get_mut(&bus_name) {
            ...
            tracing::info!("Reset player {} position to {}s after loop detection", bus_name, position_in_loop.as_secs());
        }
    }
